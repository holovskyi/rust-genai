...
pub fn generate_fake_user_id() -> String {
	// Generate 32 random bytes -> 64 hex chars (two UUID v4 worth of randomness,
	// avoiding an extra `getrandom` dependency since `uuid` is already used here).
	let mut hex_bytes = [0u8; 32];
	hex_bytes[..16].copy_from_slice(uuid::Uuid::new_v4().as_bytes());
	hex_bytes[16..].copy_from_slice(uuid::Uuid::new_v4().as_bytes());
